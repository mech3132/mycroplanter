...
gg_brmfit_all <- dat_mixedonly_exp2_withpredictions %>%
  ...
  geom_vline(aes(xintercept=0.5), col="grey", lty=2) +
  facet_grid(.~strain1,
  ...
  scale_color_manual(values=c("grey","turquoise3","skyblue3","royalblue4","black" ))+
  scale_fill_manual(values=c("grey","turquoise3","skyblue3","royalblue4","black"))+
  theme_bw() +
...
gg_brmfit_all
ggsave("03_analysis/gg_brmfit_all.png", gg_brmfit_all, height=4, width=6)
...
gg_doubledip_fluor  <- dat_forddplot %>%
  ggplot(aes(x=StrainMixsimple)) +
  geom_boxplot(aes(y=WN_FC_fluor_offset_adj2), outlier.colour = NA) +
  geom_jitter(aes(y=WN_FC_fluor_offset_adj, col=Alive), height=0, width=0.5) +
  geom_hline(aes(yintercept=0)) +
...
gg_doubledip_fluor
ggsave("03_analysis/gg_doubledip_fluor.png", gg_doubledip_fluor, height=4, width=5)
...
allCFU_sum <- allCFU %>%
  # filter(experiment == "2024-02-14") %>% 
  group_by(experiment, transfer_h, strain, plant, abs_600) %>%
